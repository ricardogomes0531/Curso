...
namespace I9Solucoes.Repositorios
{
	public class CadastroRepository : DataBase
	{
		...
		{
			bool retorno = false;
			SqlCommand query = new SqlCommand("insert into Usuario(Nome, DataNascimento, Cpf, Sexo, Email, Celular, WhatSapp, senha) values(@nome, @dataNascimento, @cpf, @sexo, @email, @celular, @whatsapp, @senha)", _conexao);
			_conexao.Open();
